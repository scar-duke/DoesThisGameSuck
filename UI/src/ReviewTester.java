...
class ReviewTester {
	...
	void testReview() {														//Assumes that there are no pre-existing reviews
		...
		Connection connection = connect.dbConnector();									//Establishes a connection so that data can be pulled from the database
		
		Review test0 = new Review(0, "GAMETEST0", 0, "USERTEST0", 3);		//Why are the names used in the constructor, rather than just the ids?
